...
describe('PostgreSQL baseline and migration system', () => {
  it('creates the exact approved platform schema from an empty database', async () => {
    await withIsolatedPostgresDatabase(async ({ database, pool }) => {
      ...
      expect(await getMigrationStatus(database)).toEqual([
        ...
        expect.objectContaining({
          name: '20260731120000_passkey_security_audit',
          executedAt: expect.any(Date),
        }),
        expect.objectContaining({
          name: '20260731130000_smtp_email_provider',
          executedAt: expect.any(Date),
        }),
      ]);
  ...
  it('rolls back the latest migration and deterministically reapplies it', async () => {
    await withIsolatedPostgresDatabase(async ({ database, pool }) => {
      ...
      expect(rolledBack.relations.map(({ name }) => name)).toContain('legacy_migration_batches');
      expect(
        rolledBack.constraints.find(({ name }) => name === 'email_channel_provider_check')
          ?.definition,
      ).not.toContain("'smtp'");
      expect(rolledBack.constraints.map(({ name }) => name)).toContain(
        'passkeys_device_type_check',
      );
      expect(rolledBack.constraints.map(({ name }) => name)).toContain('passkeys_transports_check');
  ...
  it('keeps every legacy migration, including the unsafe admin seed, outside the target runner', async () => {
    ...
    expect(targetStatusNames).toEqual([
      ...
      '20260731120000_passkey_security_audit',
      '20260731130000_smtp_email_provider',
    ]);
