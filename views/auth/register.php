<section class="max-w-md mx-auto">
  <div class="bg-white rounded-2xl shadow-glass p-6">
    <h1 class="text-xl font-semibold mb-4"><?= __('auth.register.title') ?></h1>
    ...
    <form method="post" action="/register" class="space-y-3">
      ...
      <div>
        <label class="block text-sm mb-1"><?= __('auth.register.name') ?></label>
        <input name="full_name" class="w-full rounded-xl border-gray-300" />
      </div>
      <div>
        <label class="block text-sm mb-1"><?= __('auth.register.email') ?></label>
        <input name="email" type="email" class="w-full rounded-xl border-gray-300" required />
      </div>
      <div>
        <label class="block text-sm mb-1"><?= __('auth.register.password') ?></label>
        <input name="password" type="password" class="w-full rounded-xl border-gray-300" required />
      </div>
      <button class="w-full bg-gray-900 text-white rounded-xl py-2.5"><?= __('auth.register.button') ?></button>
    </form>
  </div>
</section>