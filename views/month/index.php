...
<section class="grid md:grid-cols-3 gap-4">
  ...
  <div class="bg-white rounded-2xl p-5 shadow-glass md:col-span-2">
    ...
    <form class="grid sm:grid-cols-6 gap-2" method="post" action="/months/tx/add">
      ...
      <input name="occurred_on" type="date" value="<?= $ym ?>-01" class="rounded-xl border-gray-300" />
...
<!-- Transactions table (same edit/delete forms as before) -->
<section class="mt-6 bg-white rounded-2xl p-5 shadow-glass overflow-x-auto">
  ...
  <table class="min-w-full text-sm">
    <thead><tr class="text-left border-b">
      <th class="py-2 pr-3">Date</th><th class="py-2 pr-3">Kind</th>
      <th class="py-2 pr-3">Category</th><th class="py-2 pr-3">Amount</th>
      <th class="py-2 pr-3">Note</th><th class="py-2 pr-3">Actions</th></tr></thead>
    <tbody>
      ...
      <tr class="border-b hover:bg-gray-50">
        ...
        <td class="py-2 pr-3 font-medium"><?= moneyfmt($row['amount'], $row['currency']) ?></td>
        ...
        <td class="py-2 pr-3">
          <details><summary class="cursor-pointer text-accent">Edit</summary>
            <form class="mt-2 grid sm:grid-cols-6 gap-2" method="post" action="/months/tx/edit">
              ...
              <input name="occurred_on" type="date" value="<?= $row['occurred_on'] ?>" class="rounded-xl border-gray-300" />
              <input name="note" value="<?= htmlspecialchars($row['note'] ?? '') ?>" class="rounded-xl border-gray-300" />
