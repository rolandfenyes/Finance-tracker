  ...
  <div class="mt-6 grid md:grid-cols-2 gap-6">
          ...
          <li class="py-3 flex items-center justify-between">
            <div>
              <div class="font-medium">
                ...
                <?php if ($c['is_main']): ?><span class="ml-2 text-xs px-2 py-0.5 rounded-full bg-gray-900 text-white"><?= __('Main') ?></span><?php endif; ?>
              </div>
            ...
            <div class="flex gap-2">
              <?php if (!$c['is_main']): ?>
                <form method="post" action="/settings/currencies/main">
                  ...
                  <button class="px-3 py-1.5 rounded-lg border hover:bg-gray-50"><?= __('Set main') ?></button>
                </form>
                <form method="post" action="/settings/currencies/remove" onsubmit="return confirm('<?= addslashes(__('Remove currency :code?', ['code' => $c['code']])) ?>')">
                  <input type="hidden" name="csrf" value="<?= csrf_token() ?>" />
                  <input type="hidden" name="code" value="<?= htmlspecialchars($c['code']) ?>" />
                  <button class="px-3 py-1.5 rounded-lg border text-red-600 hover:bg-red-50"><?= __('Remove') ?></button>
                </form>
              <?php endif; ?>
            </div>
          </li>
        <?php endforeach; if (!count($userCurrencies)): ?>
          <li class="py-3 text-sm text-gray-500"><?= __('No currencies yet.') ?></li>
        <?php endif; ?>
      </ul>
    </div>
    ...
    <div>
      ...
      <form method="post" action="/settings/currencies/add" class="flex gap-2">
        ...
        <button class="px-4 rounded-xl bg-gray-900 text-white"><?= __('Add') ?></button>
      </form>
